 ...
 *
 * These are TEST accounts for a pre-launch pilot. They are seeded from
 * environment variables where available so that a deployed preview is not
 * protected by credentials sitting in a public Git repository; the fallback
 * passwords below are for local development only and are documented in the
 * README as such.
 *
...
/**
 * Local-development credentials. Deliberately obvious rather than
 * realistic-looking, so nobody mistakes a seeded row for a real partner.
 */
const DEV_ACCOUNTS: SeededAccount[] = [
  {
    ...
    region: "northern",
    password: process.env.PORTAL_DEV_PASSWORD ?? "safeher-dev-only",
  },
  {
...
/** Seeded account emails, shown on the login screen while this is a mock. */
export const SEEDED_EMAILS = DEV_ACCOUNTS.map((a) => a.email);
